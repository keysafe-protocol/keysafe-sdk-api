...
typedef struct ms_ecall_aes_gcm_128_encrypt_t {
	sgx_status_t ms_retval;
	const uint8_t* ms_plaintext;
	size_t ms_len;
	uint8_t* ms_ciphertext;
	uint8_t* ms_mac;
} ms_ecall_aes_gcm_128_encrypt_t;

typedef struct ms_ecall_aes_gcm_128_decrypt_t {
	sgx_status_t ms_retval;
...
static sgx_status_t SGX_CDECL sgx_ecall_aes_gcm_128_encrypt(void* pms)
{
	CHECK_REF_POINTER(pms, sizeof(ms_ecall_aes_gcm_128_encrypt_t));
	//
	// fence after pointer checks
	//
	sgx_lfence();
	ms_ecall_aes_gcm_128_encrypt_t* ms = SGX_CAST(ms_ecall_aes_gcm_128_encrypt_t*, pms);
	...
	size_t _len_ciphertext = _tmp_len;
	...
	ms->ms_retval = ecall_aes_gcm_128_encrypt((const uint8_t*)_in_plaintext, _tmp_len, _in_ciphertext, _in_mac);
	if (_in_ciphertext) {
	...
	if (_in_mac) {
		if (memcpy_s(_tmp_mac, _len_mac, _in_mac, _len_mac)) {
			status = SGX_ERROR_UNEXPECTED;
			goto err;
		}
	}
...
static sgx_status_t SGX_CDECL sgx_ecall_aes_gcm_128_decrypt(void* pms)
{
	CHECK_REF_POINTER(pms, sizeof(ms_ecall_aes_gcm_128_decrypt_t));
	//
	// fence after pointer checks
	//
	sgx_lfence();
	ms_ecall_aes_gcm_128_decrypt_t* ms = SGX_CAST(ms_ecall_aes_gcm_128_decrypt_t*, pms);
	sgx_status_t status = SGX_SUCCESS;
	const uint8_t* _tmp_ciphertext = ms->ms_ciphertext;
	size_t _tmp_len = ms->ms_len;
	size_t _len_ciphertext = _tmp_len;
	uint8_t* _in_ciphertext = NULL;
	uint8_t* _tmp_mac = ms->ms_mac;
	...
	ms->ms_retval = ecall_aes_gcm_128_decrypt((const uint8_t*)_in_ciphertext, _tmp_len, _in_mac, _in_plaintext);
	if (_in_plaintext) {
...
static sgx_status_t SGX_CDECL sgx_seal(void* pms)
{
	...
	sgx_status_t status = SGX_SUCCESS;
	uint8_t* _tmp_content = ms->ms_content;
	uint32_t _tmp_content_size = ms->ms_content_size;
	size_t _len_content = _tmp_content_size;
	uint8_t* _in_content = NULL;
	uint8_t* _tmp_sealed_content = ms->ms_sealed_content;
	uint32_t _tmp_sealed_size = ms->ms_sealed_size;
	size_t _len_sealed_content = _tmp_sealed_size;
	uint8_t* _in_sealed_content = NULL;

	CHECK_UNIQUE_POINTER(_tmp_content, _len_content);
	CHECK_UNIQUE_POINTER(_tmp_sealed_content, _len_sealed_content);
	...
	if (_tmp_content != NULL && _len_content != 0) {
		...
		_in_content = (uint8_t*)malloc(_len_content);
		if (_in_content == NULL) {
			status = SGX_ERROR_OUT_OF_MEMORY;
			goto err;
		}
	...
	if (_tmp_sealed_content != NULL && _len_sealed_content != 0) {
		if ( _len_sealed_content % sizeof(*_tmp_sealed_content) != 0)
		{
			status = SGX_ERROR_INVALID_PARAMETER;
			goto err;
		}
		if ((_in_sealed_content = (uint8_t*)malloc(_len_sealed_content)) == NULL) {
			status = SGX_ERROR_OUT_OF_MEMORY;
			goto err;
		}

		memset((void*)_in_sealed_content, 0, _len_sealed_content);
	}

	ms->ms_retval = seal(_in_content, _tmp_content_size, _in_sealed_content, _tmp_sealed_size);
	if (_in_sealed_content) {
		if (memcpy_s(_tmp_sealed_content, _len_sealed_content, _in_sealed_content, _len_sealed_content)) {
			status = SGX_ERROR_UNEXPECTED;
			goto err;
		}
	}
...
err:
	...
	if (_in_sealed_content) free(_in_sealed_content);
	return status;
}
...
SGX_EXTERNC const struct {
	...
	5,
	{
		{(void*)(uintptr_t)sgx_ecall_aes_gcm_128_encrypt, 0, 0},
		{(void*)(uintptr_t)sgx_ecall_aes_gcm_128_decrypt, 0, 0},
